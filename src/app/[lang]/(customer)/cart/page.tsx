...
export default function CartPage({
  ...
  const currentUserCart = useCartStore((state) => {
    return state.userCarts[userId] || null;
  });
  
  const items: CartItem[] = useMemo(() => {
    if (!userId) return [];
    return currentUserCart ? [...currentUserCart] : [];
  }, [currentUserCart, userId]);
  ...
  return (
    <main id="main-content" className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-2 inline-flex items-center gap-2" role="group" aria-label={`${dict.common.actionsFor} ${item.name}`}>
                  <Button 
                    ...
                    aria-label={`${dict.common.decreaseQuantity} ${item.name}`}
                  >
                    -
                  </Button>
                  <span className="text-sm font-medium w-6 text-center" aria-label={`${dict.cart.quantity}: ${item.quantity}`}>
                    {item.quantity}
                  </span>
                  <Button 
                    variant="secondary" 
                    ...
                    aria-label={`${dict.common.increaseQuantity} ${item.name}`}
                  >
                    +
                  </Button>
                  <Button 
                    ...
                    aria-label={`${dict.cart.remove} ${item.name} ${dict.common.fromCart}`}
                  >
                    {dict.cart.remove}
                  </Button>
                </div>
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button 
            ...
            aria-label={dict.cart.createOrder}
          >
            {dict.cart.checkout}
          </Button>
        </div>
      ...
      {showSuccess && (
        <Message
          type="success"
          message={dict.cart.orderSuccess}
